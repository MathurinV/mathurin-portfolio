<script lang="ts">
	...
	const { slice }: SliceComponentProps<Content.HeroSlice> = $props();
</script>
	...
	>
		<div>
			{#if isFilled.image(slice.primary.image)}
				<PrismicImage field={slice.primary.image} class="es-fullpage-hero__image" />
			{/if}
		</div>
		<div class="es-fullpage-hero__content-right">
			<div class="es-fullpage-hero__content__intro">
				...
				{/if}
				<PrismicLink field={slice.primary.callToActionLink} class="es-call-to-action__link" />
			</div>
		</div>
	</div>
